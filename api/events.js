...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const b = req.body
    const rows = await sql`insert into events (title,date_str,day_str,month_str,lieu,type_str,descr,full_descr,published)
  ...
  if (req.method === 'PATCH') {
    const b = req.body
    await sql`update events set title=${b.title},date_str=${b.date},day_str=${b.day},
      month_str=${b.month},lieu=${b.lieu},type_str=${b.type},descr=${b.desc},
      full_descr=${b.full_desc},published=${b.published!==false} where id=${b.id}`
